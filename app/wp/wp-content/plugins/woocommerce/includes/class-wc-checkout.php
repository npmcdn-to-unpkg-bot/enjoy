	...
	public function __construct () {
		...
		$this->checkout_fields['order']	= array(
			'order_comments' => array(
				'type' => 'textarea',
				'class' => array('notes'),
				'label' => __( 'Ваші побажання щодо замовлення', 'woocommerce' ),
				'placeholder' => _x('Напишіть ваші особливі побажання щодо замовлення.', 'placeholder', 'woocommerce')
			)
		);
			...
			foreach ( $this->checkout_fields as $fieldset_key => $fieldset ) {
				...
				foreach ( $fieldset as $key => $field ) {
					...
					if ( isset( $field['required'] ) && $field['required'] && empty( $this->posted[ $key ] ) ) {
						wc_add_notice( '<strong>' . $field['label'] . '</strong> ' . __( 'потрібне поле.', 'woocommerce' ), 'error' );
					}
