	...
	public static function save_address() {
		...
		foreach ( $address as $key => $field ) {
			...
			if ( ! empty( $field['required'] ) && empty( $_POST[ $key ] ) ) {
				wc_add_notice( $field['label'] . ' ' . __( 'потрібне поле.', 'woocommerce' ), 'error' );
			}
	...
	public static function save_account_details() {
		...
		foreach ( $required_fields as $field_key => $field_name ) {
			if ( empty( $_POST[ $field_key ] ) ) {
				wc_add_notice( '<strong>' . esc_html( $field_name ) . '</strong> ' . __( 'потрібне поле.', 'woocommerce' ), 'error' );
			}
		}
	...
	public static function update_cart_action() {
		...
		elseif ( ! empty( $_GET['remove_item'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'woocommerce-cart' ) ) {
			...
			if ( $cart_item = WC()->cart->get_cart_item( $cart_item_key ) ) {
				...
				$item_removed_title = apply_filters( 'woocommerce_cart_item_removed_title', $product ? $product->get_title() : __( 'шт.', 'woocommerce' ), $cart_item );
		...
		if ( ( ! empty( $_POST['apply_coupon'] ) || ! empty( $_POST['update_cart'] ) || ! empty( $_POST['proceed'] ) ) && isset( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-cart' ) ) {
			...
			if ( ! empty( $_POST['proceed'] ) ) {
				...
				wc_add_notice( __( 'Кошик оновлено.', 'woocommerce' ) );
		...
		if ( ! empty( $_POST['login'] ) && ! empty( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-login' ) ) {
			...
			try {
				...
				if ( is_wp_error( $user ) ) {
					...
					// Feedback
					wc_add_notice( sprintf( __( 'Ви зайшли як <strong>%s</strong>', 'woocommerce' ), $user->display_name ) );
