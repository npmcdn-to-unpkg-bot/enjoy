	...
	public function get_order_item_totals( $tax_display = '' ) {
		...
		if ( $this->get_total() > 0 && $this->payment_method_title ) {
			$total_rows['payment_method'] = array(
				'label' => __( 'Метода оплати:', 'woocommerce' ),
		...
		$total_rows['order_total'] = array(
			'label' => __( 'Загальна сума:', 'woocommerce' ),
				...
				if ( $item['product_id'] > 0 ) {
					...
					if ( $_product && $_product->exists() && $_product->managing_stock() ) {
						...
						if ( isset( $item['variation_id'] ) && $item['variation_id'] ) {
							$this->add_order_note( sprintf( __( 'Шт. %s variation #%s stock reduced from %s to %s.', 'woocommerce' ), $item_name, $item['variation_id'], $new_stock + $qty, $new_stock) );
						} else {
							$this->add_order_note( sprintf( __( 'Шт. %s stock reduced from %s to %s.', 'woocommerce' ), $item_name, $new_stock + $qty, $new_stock) );
						}
