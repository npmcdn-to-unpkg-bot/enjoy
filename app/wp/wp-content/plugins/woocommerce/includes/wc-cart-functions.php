...
function wc_add_to_cart_message( $product_id ) {
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message   = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'Переглянути кошик', 'woocommerce' ), esc_html( $added_text ) );
	}
