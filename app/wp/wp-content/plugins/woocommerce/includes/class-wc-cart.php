			...
			try {
				...
				if ( $product_data->is_sold_individually() ) {
					...
					if ( $in_cart_quantity > 0 ) {
						throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(), __( 'Переглянути кошик', 'woocommerce' ), sprintf( __( 'You cannot add another &quot;%s&quot; to your cart.', 'woocommerce' ), $product_data->get_title() ) ) );
					}
				}
				...
				if ( $managing_stock = $product_data->managing_stock() ) {
					...
					if ( ! $product_data->has_enough_stock( $check_qty + $quantity ) ) {
						throw new Exception( sprintf(
							...
							wc_get_cart_url(),
							__( 'Переглянути кошик', 'woocommerce' ),
