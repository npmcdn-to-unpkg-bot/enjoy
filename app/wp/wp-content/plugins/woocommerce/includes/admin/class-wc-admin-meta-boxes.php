...
class WC_Admin_Meta_Boxes {
	...
	public function add_meta_boxes() {
		...
		foreach ( wc_get_order_types( 'order-meta-boxes' ) as $type ) {
			...
			add_meta_box( 'woocommerce-order-data', sprintf( __( '%s Дата', 'woocommerce' ), $order_type_object->labels->singular_name ), 'WC_Meta_Box_Order_Data::output', $type, 'normal', 'high' );
			add_meta_box( 'woocommerce-order-items', sprintf( __( '%s шт.', 'woocommerce' ), $order_type_object->labels->singular_name ), 'WC_Meta_Box_Order_Items::output', $type, 'normal', 'high' );
	...
	public function rename_meta_boxes() {
		...
		if ( isset( $post ) && ( 'publish' == $post->post_status || 'private' == $post->post_status ) ) {
			...
			add_meta_box( 'commentsdiv', __( 'Відгук', 'woocommerce' ), 'post_comment_meta_box', 'product', 'normal' );
		}
	}
