...
class WC_Admin_Post_Types {
	...
	public function shop_coupon_columns( $existing_columns ) {
		...
		$columns['description'] = __( 'Опис', 'woocommerce' );
	...
	public function shop_order_columns( $existing_columns ) {
		...
		$columns['order_title']      = __( 'Замовлення', 'woocommerce' );
		...
		$columns['order_notes']      = '<span class="order-notes_head tips" data-tip="' . esc_attr__( 'Ваші побажання', 'woocommerce' ) . '">' . esc_attr__( 'Ваші побажання щодо замовлення', 'woocommerce' ) . '</span>';
		$columns['order_date']       = __( 'Дата', 'woocommerce' );
		$columns['order_total']      = __( 'Загальна сума', 'woocommerce' );
	...
	private function _render_product_row_actions( $post, $title ) {
		...
		if ( $post_type_object->public ) {
			if ( in_array( $post->post_status, array( 'pending', 'draft', 'future' ) ) ) {
				...
				$actions['view'] = '<a href="' . get_permalink( $post->ID ) . '" title="' . esc_attr( sprintf( __( 'Переглянути &#8220;%s&#8221;', 'woocommerce' ), $title ) ) . '" rel="permalink">' . __( 'Переглянути', 'woocommerce' ) . '</a>';
			}
		}
	...
	public function render_shop_order_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'order_items' :
				...
				echo '<a href="#" class="show_order_items">' . apply_filters( 'woocommerce_admin_order_item_count', sprintf( _n( '%d шт.', '%d шт.', $the_order->get_item_count(), 'woocommerce' ), $the_order->get_item_count() ), $the_order ) . '</a>';
				...
				?><p>
					<?php
						...
						$actions['view'] = array(
							...
							'name'      => __( 'Переглянути', 'woocommerce' ),
							'action'    => "view"
						);
	...
	public function bulk_admin_footer() {
		...
		if ( 'shop_order' == $post_type ) {
			...
			jQuery(function() {
				jQuery('<option>').val('mark_processing').text('<?php _e( 'Mark у роботі', 'woocommerce' )?>').appendTo('select[name="action"]');
				jQuery('<option>').val('mark_processing').text('<?php _e( 'Mark у роботі', 'woocommerce' )?>').appendTo('select[name="action2"]');
				...
				jQuery('<option>').val('mark_completed').text('<?php _e( 'Mark виконано', 'woocommerce' )?>').appendTo('select[name="action"]');
				jQuery('<option>').val('mark_completed').text('<?php _e( 'Mark виконано', 'woocommerce' )?>').appendTo('select[name="action2"]');
			});
			</script>
			<?php
		}
	...
	public function edit_form_after_title( $post ) {
		if ( 'shop_coupon' === $post->post_type ) {
			?>
			<textarea id="woocommerce-coupon-description" name="excerpt" cols="5" rows="2" placeholder="<?php esc_attr_e( 'Опис (optional)', 'woocommerce' ); ?>"><?php echo $post->post_excerpt; // This is already escaped in core ?></textarea>
			<?php
		}
	}
